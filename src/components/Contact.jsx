...
const Contact = () => {
  ...
  const socials = [
    {
      label:'LinkedIn', href:'https://linkedin.com/in/murat-sinmez-980185225/', color:'#f59e0b',
    ...
    {
      label:'GitHub', href:'https://github.com/muratsinmez', color:'#fb923c',
  ...
  return (
    <>
      ...
      <section id="contact" className="contact-section" style={{
        ...
        marginTop:'-1px', position:'relative', zIndex:3,
      }}>
        {/* subtle grid */}
        ...
        <div style={{ position:'absolute', top:'20%', left:'50%', width:400, height:400, background:'radial-gradient(circle,rgba(245,158,11,.07) 0%,transparent 70%)', borderRadius:'50%', filter:'blur(50px)', pointerEvents:'none' }} />
        ...
        <div style={{ maxWidth:920, margin:'0 auto', position:'relative', zIndex:1 }}>
          ...
          <div style={{ textAlign:'center', marginBottom:56 }}>
            ...
            <h2 style={{ fontFamily:"'Space Mono',monospace", fontSize:'clamp(1.7rem,4vw,2.8rem)', fontWeight:700, color:'#f1f5f9', marginBottom:16 }}>
              İletişime Geç <span style={{ color:'#f59e0b' }}>_</span>
            </h2>
            ...
            <div style={{ animation:'fadeUp .5s ease' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:32 }}>
                {contactInfo.map((item,i) => (
                  <div key={i} className="info-row" style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', background:'rgba(13,27,53,.5)', border:'1px solid rgba(245,158,11,.2)', borderLeft:`3px solid ${item.color}`, transition:'border-color .25s' }}>
                    <div style={{ width:40, height:40, flexShrink:0, background:`${item.color}18`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.1rem' }}>{item.icon}</div>
                    <div>
                      <div style={{ color:'#a89278', fontSize:'.7rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'.08em', marginBottom:3 }}>{item.label}</div>
                      {item.href
                        ? <a href={item.href} style={{ color:'#f1f5f9', fontSize:'.88rem', textDecoration:'none', transition:'color .2s' }} onMouseEnter={e=>e.target.style.color=item.color} onMouseLeave={e=>e.target.style.color='#f1f5f9'}>{item.value}</a>
                        : <span style={{ color:'#f1f5f9', fontSize:'.88rem' }}>{item.value}</span>
                      }
                    </div>
                  </div>
                ))}
              ...
              <p style={{ color:'#f59e0b', fontSize:'.78rem', fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase', marginBottom:14 }}>Sosyal Medya</p>
              <div className="social-row" style={{ display:'flex', gap:10 }}>
                {socials.map((s,i) => (
                  <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" className="social-a"
                    style={{ display:'flex', alignItems:'center', gap:9, padding:'10px 16px', background:'rgba(13,27,53,.5)', border:'1px solid rgba(245,158,11,.2)', color:s.color, textDecoration:'none', fontWeight:700, fontSize:'.84rem' }}>
                    {s.icon}{s.label}
                  </a>
                ))}
              </div>
          ...
          <div style={{ textAlign:'center', marginTop:64, paddingTop:24, borderTop:'1px solid rgba(245,158,11,.15)' }}>
            <p style={{ color:'#4a3c20', fontSize:'.78rem', fontFamily:"'Space Mono',monospace" }}>
