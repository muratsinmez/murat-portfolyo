  ...
  return (
    <>
      <style>{`
        ...
        .c-input:focus { border-color: rgba(56,189,248,0.5) !important; background: rgba(15,23,42,0.95) !important; }
        .info-row:hover { border-color: rgba(56,189,248,0.2) !important; }
        .social-a { transition: all 0.25s ease; }
        .social-a:hover { transform: translateY(-3px); }
        .send-btn { transition: opacity 0.2s, transform 0.2s; }
        .send-btn:hover { opacity: 0.87; transform: translateY(-1px); }
      `}</style>

      <section id="contact" style={{
        background: 'linear-gradient(180deg, #060d1a 0%, #0a1628 100%)',
        color: '#f1f5f9', padding: '110px 32px 80px',
        fontFamily: "'DM Sans', sans-serif",
      }}>
        <div style={{ maxWidth: 920, margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <p style={{ color: '#64748b', fontSize: '0.98rem', maxWidth: 460, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 40, alignItems: 'start' }}>
            ...
            <div style={{ animation: 'fadeUp 0.5s ease' }}>
              <p style={{ color: '#64748b', fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 20 }}>
                İletişim Bilgileri
              </p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 36 }}>
                {contactInfo.map((item, i) => (
                  <div key={i} className="info-row" style={{
                    ...
                    <div>
                      <div style={{ color: '#4a5568', fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>
                        {item.label}
                      </div>
                      {item.href
                        ? <a href={item.href} style={{ color: '#e2e8f0', fontSize: '0.9rem', textDecoration: 'none', transition: 'color 0.2s' }}
              ...
              <p style={{ color: '#64748b', fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 16 }}>
                Sosyal Medya
              </p>
              <div style={{ display: 'flex', gap: 12 }}>
                {socials.map((s, i) => (
                  <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
                    ...
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.background = 'rgba(15,23,42,0.7)'; }}
                  >
                    {s.icon}{s.label}
                  </a>
                ))}
            ...
            <div style={{ animation: 'fadeUp 0.5s 0.15s ease both' }}>
              <form onSubmit={handleSubmit} style={{
                ...
                {sent && (
                  <div style={{
                    padding: '12px 18px', borderRadius: 10, marginBottom: 22,
                    background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.3)',
                    color: '#34d399', fontWeight: 600, textAlign: 'center',
                    animation: 'successPop 0.3s ease',
                  }}>✓ Mail uygulamanız açıldı!</div>
                )}
                ...
                {[
                  ...
                  <div key={f.name} style={{ marginBottom: 18 }}>
                    <label style={{ display: 'block', color: '#4a5568', fontSize: '0.78rem', fontWeight: 600, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                      {f.label}
                    </label>
                    <input
                      type={f.type} name={f.name} value={form[f.name]}
                      onChange={handleChange} placeholder={f.placeholder}
                      required className="c-input" style={inputStyle}
                    />
                  </div>
                ))}
                ...
                <div style={{ marginBottom: 22 }}>
                  ...
                  <textarea
                    ...
                    rows={5} required className="c-input"
                ...
                <button type="submit" className="send-btn" style={{
                  ...
                  boxShadow: '0 4px 20px rgba(14,165,233,0.22)',
                }}>
                  Mesaj Gönder →
                </button>
              </form>
          ...
          {/* Footer */}
          <div style={{ textAlign: 'center', marginTop: 72, paddingTop: 28, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <p style={{ color: '#1e293b', fontSize: '0.8rem', fontFamily: "'Space Mono', monospace" }}>
